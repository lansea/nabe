...
// expects a term post parameter
Renderer.prototype.search = function(req, res, next){
  ...
  this.posts.search(term, where, function(err, results) {
    if(err) { return next(err); }
    self.render('search.html', res, {articles: results, term: prefix + ' for ' + term.replace(/\[.+\]|\{.+\}/g, '')});
  });
...
Renderer.prototype.archives = function(req, res, next){
  var abspath = this.options.articleDir.replace(/\/$/, ''),
  self = this;

  this.posts.all(abspath, function(err, results) {
    if(err) { return next(err); }
    self.render('archives.html', res, {articles: results, headers: req.headers});
  });
