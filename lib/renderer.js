...
var Renderer = function Renderer(o) {
  var dir = Path.join(cwd, o.themeDir, o.theme),
  pages = [];
  ...
  config = this.options = util.extend({}, config, 
    yaml.eval(fs.readFileSync(Path.join(dir, 'config.yml')).toString()
  ));
...
Renderer.prototype.article = function(req, res, next) {
  ...
  fs.stat(mkdpath, function(err, stat) {
    ...
    self.posts.find(abspath + '.markdown', function(err, results){
      if(err) { return next(err); }
