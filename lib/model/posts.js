...
util = require('../util'),
findgit = require('../findgit'),

readFile = git.safe(function readFile(version, path, callback) {
...
processArticles = function processArticles(options, files, tag, cb) {
  ...
  files.forEach(function(file) {
    readFile('fs', file, function (err, body) {
      var props, sum;
...
Posts.prototype = {
  ...
  find: function find(path, cb) {
    ...
    if(!this.validRepo) {
      readFile('fs', path, self.handleFile.bind(self, null, path, cb));
      return this;
    }
    
    git.log(path, function(err, logs) {
      if(err) {return cb(err);}
      readFile('fs', path, self.handleFile.bind(self, logs, path, cb));
    });
